...
export type TrackOptions = {
  ...
  intro?: { endTime: number; label?: string; color?: string };
...
export type MultitrackEvents = {
  canplay: [];
  timeupdate: [number]; // Add timeupdate event with the current time as a parameter
  ...
  drop: [{ id: TrackId }];
  'region-added': [{ id: string; trackId: TrackId; start: number; end: number }];
  'region-updated': [{ id: string; trackId: TrackId; start: number; end: number }];
  'region-removed': [{ id: string; trackId: TrackId }];
};
...
class MultiTrack extends EventEmitter<MultitrackEvents> {
        ...
        if (track.regions) {
          track.regions.forEach((region) => {
            const regionInstance = wsRegions.addRegion({
              ...
              color: region.color || 'rgba(0, 0, 0, 0.3)',
              drag: region.draggable !== false,
              resize: region.resizable !== false,
            });

            // Emit events for region updates
  ...
  private updatePosition(time: number, autoCenter = false) {
    ...
    if (time !== this.currentTime) {
      ...
      // Emit the timeupdate event with the current time
  ...
  public addRegion(trackId: TrackId, region: RegionOptions) {
    ...
    if (wsRegions) {
      ...
      this.tracks[trackIndex].regions = this.tracks[trackIndex].regions || [];
      this.tracks[trackIndex].regions!.push(region);

      this.emit('region-added', { id: region.id, trackId, start: region.start, end: region.end });
      ...
      regionInstance.on('dblclick', () => {
        if (confirm(`Remove region ${region.id}?`)) {
          regionInstance.remove();
          this.tracks[trackIndex].regions = this.tracks[trackIndex].regions?.filter((r) => r.id !== region.id);
          this.emit('region-removed', { id: region.id, trackId });
        }
      });
    }
  }

  public removeRegion(trackId: TrackId, regionId: string) {
    const trackIndex = this.tracks.findIndex((t) => t.id === trackId);
    if (trackIndex === -1) return;
    ...
    if (wsRegions) {
      ...
      if (region) {
        region.remove();
        this.tracks[trackIndex].regions = this.tracks[trackIndex].regions?.filter((r) => r.id !== regionId);
        this.emit('region-removed', { id: regionId, trackId });
      }
    }
  }
}
